function   [et,etp,hyprad]=hyR_tri(r,ns)
% hyR_tri.m
...
% Computing the hyperbolic rad
Trhyprad= @(r) rhoB(verT(1),verT(2));
% Computing the Euclidean center: see HKVbook (4.20);
EuCent = @(x,M)((1-(tanh(M/2))^2)* x./(1-((tanh(M/2))^2)*abs(x).^2));
%
% The hyperbolic rad 
hyprad=Trhyprad(r);
% The Euclidean center 
Eucent=EuCent(r,hyprad);
cv = [exp(2i*pi/3) ; 1 ; exp(-2i*pi/3)].*Eucent;
% The Euclidean radius
Eurad=abs(Eucent- r*exp(i*2*pi/3));
%
